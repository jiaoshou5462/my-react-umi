...
let addEditCard = (props) => {
  let { dispatch, moneyQuato, selsctMarketingItems, isCardRadioTabs, compData, allObj, editInfo } = props;
  ...
  let [temType, setTemType] = useState(null);//模版类型（1：简易模版，2：保险公司模版）

  let [cardPackageFlag, setCardPackageFlag] = useState(history.location.query.cardPackageFlag);//0卡券，1卡包
  let [identification, setIdentification] = useState(null);//发放标识
  let [fileId, setFileId] = useState('');//文件存放ID

  // 卡券
  let [totalPeople, setTotalPeople] = useState(0)//总人数
  let [totalNum, setTotalNum] = useState(0)//总发放数量
  const [receiveType, setReceiveType] = useState(null)//卡包领取方式

  // 1card领取方式
  let onChangeReceiveWay = (e) => {
    setReceiveType(e.target.value);
  }

  let grantBatchId = history.location.query.grantBatchId;//批次id
  useEffect(() => {
    if (grantBatchId) {
      getStartInfo();
    }
  ...
  const getStartInfo = () => {
    dispatch({
      ...
      callback: (res) => {
        setFileId(res.body.fileId);
        setFileName(res.body.fileName);
        setMarketProjectId(res.body.marketProjectId);
        setIdentification(res.body.identification);
        setTemType(res.body.templateType)
        setReceiveType(res.body.cardReceiveType)
        setShareFlag(res.body.shareFlag)
      }
    })
  }

  let [isChangeDate, setIsChangeDate] = useState(false);//是否改变过卡包时间
  // 2时间选择框改变时间，外面
  let changeCardTime = (e) => {
    setIsChangeDate(true);
    if (e) {
      effectiveStartDate = moment(e[0]);
      setEffectiveStartDate(effectiveStartDate)
      effectiveEndDate = moment(e[1])
  ...
  useEffect(() => {
    if (Object.keys(editInfo).length > 0) {
      let endDate = moment(editInfo.effectiveEndDate);
      let startDate = moment(editInfo.effectiveStartDate);

      let data = {
        ...editInfo,
        marketProjectId: {
          key: editInfo.marketProjectId,
          value: editInfo.marketProjectId,
          label: editInfo.marketProjectName,
        },
        effectiveDate: isChangeDate ? [effectiveStartDate, effectiveEndDate] : [startDate, endDate],
      }
      ...
      let newData = {
        ...editInfo,
        marketProjectId: editInfo.marketProjectId,
        marketProjectName: editInfo.marketProjectName,
        grantName: editInfo.grantName,
        effectiveEndDate: editInfo.cardPackageFlag == 2 ? editInfo.effectiveEndDate : null,
        effectiveStartDate: editInfo.cardPackageFlag == 2 ? editInfo.effectiveStartDate : null,
      }
      dispatch({
  ...
  let [effectiveStartDate, setEffectiveStartDate] = useState('');//卡券,卡包有效期开始时间
  let [effectiveEndDate, setEffectiveEndDate] = useState('');//卡券,卡包有效期结束时间
  ...
  let marketProjectIdOrName = (e) => {
    if (e) {
      setMarketProjectId(e.value)//id
      setMarketProjectName(e.label)//name
      setMarketProjectNameDate(e.label + '_' + moment().format('YYYYMMDD'))//所属项目name_20210908
      form.setFieldsValue({ grantName: e.label + '_' + moment().format('YYYYMMDD') })
    }
  }
  ...
  let [isRealUsed, setIsRealUsed] = useState(true);//编辑一进入时有效
  // 点击保存
  let subBtnEvent = (values) => {
    ...
    if (values.effectiveDate) {
      // console.log(values.effectiveDate, 'values.effectiveDate')
    ...
    if (cardPackageFlag == 0) {//0卡券
      ...
      // 上传文件判断
      if (!isUseFile) {//无效的名单(1编辑进入时，2上传无效名单时)
        if (totalPeople && totalNum) {//重新上传时
          message.warning({ content: '请重新上传有效名单！' })
        } else {//编辑时直接保存时
          // if ((moneyQuato.allQuoat - moneyQuato.useQuoat - moneyQuato.occupyQuoat) < allObj.allMoney * editInfo.priceCount) {
          //   message.warning({ content: '本月投放额度不足，请联系管理员' })
          // } else if (allObj.allCont * editInfo.priceCount > 100000) {
          //   message.warning({ content: '单批次导入卡券总张数不能超过100000张' })
          // }
        }
      } else {
        // if ((moneyQuato.allQuoat - moneyQuato.useQuoat - moneyQuato.occupyQuoat) < allObj.allMoney * totalNum) {
        //   message.warning({ content: '本月投放额度不足，请联系管理员' })
        // } else if (allObj.allCont * totalNum > 100000) {
        //   message.warning({ content: '单批次导入卡券总张数不能超过100000张' })
        // }
      }
      // && ((moneyQuato.allQuoat - moneyQuato.useQuoat - moneyQuato.occupyQuoat) - allObj.allMoney * totalNum) > 0
      if (marketProjectId && values.grantName && values.identification && compData.detailList.length && fileId && (isUseFile || isRealUsed)) {
        setIsSaveVisible(true)
        setMarketProjectId(values.marketProjectId.value)
        setMarketProjectName(values.marketProjectId.label)
      ...
      // } 
      // 上传文件判断
      if (!isRealUsed && !isUseFile) {
        message.warning({ content: '请重新上传有效名单！' })
      }
      if (marketProjectId && values.marketProjectId.label && values.grantName && values.identification && compData.detailList.length && fileId && (isUseFile || isRealUsed)) {
        setIsSaveVisible(true)
        setMarketProjectId(values.marketProjectId.value)
        setMarketProjectName(values.marketProjectId.label)
        setGrantName(values.grantName)
      ...
      if (marketProjectId && values.marketProjectId.label && values.grantName && values.verifyNum && compData.detailList.length) {
        setIsSaveVisible(true)
        setMarketProjectId(values.marketProjectId.value)
        setMarketProjectName(values.marketProjectId.label)
        setGrantName(values.grantName)
      ...
      if (marketProjectId && values.marketProjectId.label && values.grantName && values.identification && fileId && (isUseFile || isRealUsed) && values.verifyNum && compData.detailList.length) {
        setIsSaveVisible(true)
        setMarketProjectId(values.marketProjectId.value)
        setMarketProjectName(values.marketProjectId.label)
        setGrantName(values.grantName)
      ...
      if (marketProjectId && values.marketProjectId.label && values.grantName && compData.detailList.length) {
        setIsSaveVisible(true)
        setMarketProjectId(values.marketProjectId.value)
        setMarketProjectName(values.marketProjectId.label)
        setGrantName(values.grantName)
  ...
  // 二次保存
  let handleSaveOk = () => {
    // console.log(compData, 'compData222') return;
    dispatch({
      ...
      payload: {
        ...
        params: {
          ...
          grantName: grantName,//批次名称
          cardPackageFlag: cardPackageFlag,//0卡券，1卡包,2兑换码，3N选M，4接口投放
          grantType: 2,//发放类型 1：活动 2：直投

          identification: (cardPackageFlag == 0 || cardPackageFlag == 1 || cardPackageFlag == 3) ? identification : null,//发放标识  1.客户手机号 2.身份证号码 3微信openId 4.车架号+车牌号
  ...
  return (
    <>
        ...
        <Form className={style.form__cont} form={form} onFinish={subBtnEvent}>
          <div style={{ marginLeft: '-70px' }}>
            ...
            <Form.Item name="grantName" label="批次名称：" className={style.form__item} labelCol={{ span: 8 }} rules={[{ required: true, message: '此项不能为空' }]}>
              <Input placeholder="请输入批次名称" maxLength={50} style={{ width: '66%' }} onChange={grantNameChange} value={marketProjectNameDate}></Input>
            </Form.Item>
                      ...
                      <>
                        <div style={{ display: 'flex' }}>
                          <Form.Item label="模板名单：" style={{ marginTop: '20px' }} className={style.form__item} labelCol={{ span: 8 }}>
                            ...
                            {
                              isUseFile == '0' || editInfo.count ?
                                <span style={{ color: '#B9B9B9', margin: '0 20px' }}>{totalPeople ? totalPeople : editInfo.count}人</span>
                                : ''
                            }
                          </Form.Item>
                        </div>
                        {
                          isUseFile == '0' || editInfo.fileName ?
                            <div style={{ marginLeft: "230px", marginBottom: '20px', color: '#B9B9B9' }}>{fileName ? fileName : editInfo.fileName}</div>
                            :
                      ...
                      <>
                        ...
                        </div>
                        {
                          fileName || editInfo.fileName ?
                            <div style={{ marginLeft: "220px", marginBottom: '20px', color: '#B9B9B9' }}>{fileName ? fileName : editInfo.fileName}</div>
                            :
                            <div style={{ marginLeft: "230px" }}></div>
                        }
                        <a style={{ lineHeight: '22px', color: '#1890ff', marginLeft: "220px", cursor: "pointer" }}
            ...
            {
              cardPackageFlag == 0 ?
                // <Form.Item label="卡券领取有效期：" name="effectiveDate" className={style.form__item} style={{ marginTop: '20px' }} labelCol={{ span: 8 }} rules={[{ required: true, message: '此项不能为空' }]}>
                //   <RangePicker
                ...
                cardPackageFlag == 1 ?
                  <>
                    ...
                    <Form.Item name="shareFlag" label="是否支持转赠：" className={style.form__item} labelCol={{ span: 8 }} rules={[{ required: true, message: '此项不能为空' }]}>
                  ...
                  cardPackageFlag == 2 ?
                    <>
                      ...
                      <Form.Item label="卡券兑换有效期：" name="effectiveDate" className={style.form__item} labelCol={{ span: 8 }} rules={[{ required: true, message: '此项不能为空' }]}>
                        <RangePicker
                          placeholder={['开始时间', '结束时间']}
                          disabledDate={disabledDate}
                        />
                    ...
                    : cardPackageFlag == 3 ?
                      <>
                        ...
                        <Form.Item name="verifyNum" label="最多领取卡券数：" className={style.form__item} labelCol={{ span: 8 }} rules={[{ required: true, message: '此项不能为空' }]}>
                          <InputNumber placeholder="填写用户可领取数量"
                            min={1}
          ...
          <div className={style.block__cont__t}>
            ...
            {
              cardPackageFlag == 0 || cardPackageFlag == 3 ? <Coupon editInfo={editInfo} detailList={editInfo && editInfo.detailList} /> :
                cardPackageFlag == 1 ?
                  <Cardbag
                    ...
                    editInfo={editInfo}
                    cardPackageEditResults={editInfo && editInfo.cardPackageEditResults}
                    packageAddResult={editInfo && editInfo.packageAddResult}
                    cardReceiveType={receiveType}
                    shareFlag={shareFlag}
                    effectiveStartDate={effectiveStartDate}
                    effectiveEndDate={effectiveEndDate}
                  />
                  :
                  cardPackageFlag == 2 ? <ExchangeCode editInfo={editInfo} detailList={editInfo && editInfo.detailList} /> :
                    cardPackageFlag == 4 ? <InterfaceLaunch editInfo={editInfo} detailList={editInfo && editInfo.detailList} /> :
                      ''
            }
      ...
      {/* 二次保存框 */}
      <Modal Modal visible={isSaveVisible} style={{ textAlign: 'center' }} okText='提交' onOk={handleSaveOk} onCancel={() => { setIsSaveVisible(false) }}>
        {
          cardPackageFlag == 0 ?
            <div>
              <h3 style={{ fontSize: '16px' }}>请确认提交本次卡券投放</h3>
              <p style={{ marginTop: '40px' }}>
                本次卡券投放人数 {totalPeople ? totalPeople : editInfo.count} 人，
                共 {totalNum ? allObj.allCont * totalNum : allObj.allCont * editInfo.priceCount} 张卡券
              </p>
            </div> :
            cardPackageFlag == 1 ?
              <div>
                <h3 style={{ fontSize: '16px' }}>请确认提交本次卡包投放</h3>
                <p style={{ marginTop: '40px' }}>本次卡券投放人数 {totalPeople ? totalPeople : editInfo.count} 人，共 {totalNum ? totalNum : editInfo.priceCount} 个卡包</p>
              </div> :
              cardPackageFlag == 2 ?
                <div>
                ...
                cardPackageFlag == 3 ?
                  <div>
                    ...
                    <p style={{ marginTop: '40px' }}>本次卡券投放人数 {totalPeople ? totalPeople : editInfo.count} 人，共 {totalPeople ? allObj.allCont * totalPeople : allObj.allCont * editInfo.count} 张卡券</p>
                    {/* <p style={{ marginTop: '40px' }}>本次卡券投放人数 {totalPeople ? totalPeople : editInfo.count} 人，共 {totalNum ? allObj.allCont * totalNum : allObj.allCont * editInfo.priceCount} 张卡券</p> */}
                  </div> :
                  cardPackageFlag == 4 ?
                    <div>
...
export default connect(({ cardgrantManageModel }) => ({
  ...
  editInfo: cardgrantManageModel.editInfo
}))(addEditCard)
