...
const DetailRealed = (props) => {
  ...
  let realColumns = [
    ...
    { title: '工单号', dataIndex: 'thirdOrderNo', key: 'thirdOrderNo' },
    { title: '订单日期', dataIndex: 'orderCreateTime', key: 'orderCreateTime' },
    ...
    {
      title: '轨迹', dataIndex: 'trajectory', key: 'trajectory',
      render: (text, all) => {
    ...
    { title: '服务状态', dataIndex: 'orderStatusName', key: 'orderStatusName' },
    // {
    //   title: '服务类型',
  ...
  return (
    <>
      <div className={style.block__cont}>
        ...
        <div className={style.form__cont}>
          ...
          <div className={style.form__cont}>
            ...
            <Row justify="space-around" align="center">
              <Col className={style.form__item} span={8}>
                <div>结算笔数：<span>{channelBalanceBillInfo.confirmedCount}</span></div>
              </Col>
              <Col className={style.form__item} span={8}>
      ...
      <div className={style.block__cont}>
        <Form className={style.form__cont} form={form} onFinish={searchRealBtn}>
          <h3 style={{ marginBottom: '50px' }}>订单查询</h3>
          <Row justify="space-around" align="center">
            <Form.Item label="订单时间：" name='orderTime' className={style.form__item} labelCol={{ span: 8 }}>
              <RangePicker locale={locale} placeholder={['开始时间', '结束时间']} />
            </Form.Item>
            <Form.Item label="订单编号：" name="orderNo" className={style.form__item} labelCol={{ span: 8 }}>
              <Input placeholder="请输入" ></Input>
            </Form.Item>
            <Form.Item label="车牌号：" name="plateNo" className={style.form__item} labelCol={{ span: 8 }}>
              <Input placeholder="请输入" ></Input>
            </Form.Item>
          </Row>
          {/* <Row justify="space-around" align="center">
            <Form.Item label="服务状态：" name="orderStatus" className={style.form__item} labelCol={{ span: 8 }}>
              <Select placeholder="不限" allowClear>
                {
                  caseStatusList.map((v) => <Option key={v.caseStatus} value={v.caseStatus}>{v.caseStatusName}</Option>)
                }
              </Select>
            </Form.Item>
            <Form.Item label="服务类型：" name="serviceTypeId" className={style.form__item} labelCol={{ span: 8 }}>
              <Select onChange={(e) => { clickCaseServiceList(e) }} placeholder="不限" allowClear>
                {
                  caseServiceList.map((v) => <Option key={v.serviceTypeId} value={v.serviceTypeId + '&' + JSON.stringify(v.caseServiceItems)}>{v.serviceTypeName}</Option>)
                }
              </Select>
            </Form.Item>
            <Form.Item label="服务项目：" name="serviceId" className={style.form__item} labelCol={{ span: 8 }}>
              <Select placeholder="不限" disabled={isDisable} allowClear
                showSearch
              ...
              >
                {
                  caseServiceItems.map((v) => <Option key={v.serviceId} value={v.serviceId}>{v.serviceName}</Option>)
                }
              </Select>
            </Form.Item>
      ...
      <div className={style.block__cont__t}>
        <div className={style.listTitle}>
          <span style={{ padding: '30px 20px' }}>订单明细</span>
        ...
        <Table columns={realColumns} dataSource={infoDetailList} pagination={false}></Table>
        <Pagination
