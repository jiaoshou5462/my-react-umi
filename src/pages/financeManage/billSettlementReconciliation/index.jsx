...
// 结算对账
const billSettlementReconciliation = (props) => {
  ...
  // 搜索
  let searchBtn = (val) => {
    if (!val.date || getMonthRange(val.date[0], val.date[1]) > 11) {
      message.warning('结算时间跨度不能超过12个月!'); // 传入年月日去判断  大于12个月给出提示;
      return false;
    }
  ...
  //账单明细
  let realColumns = [
    { title: '结算时间', dataIndex: 'dateStr', key: 'dateStr', render: (dateStr) => <ListTableTime>{dateStr}</ListTableTime> },
    ...
    { title: '待确认(笔)', dataIndex: 'customerUnconfirmedCount', key: 'customerUnconfirmedCount', render: (customerUnconfirmedCount, record) => renderTransactions(customerUnconfirmedCount, record, 2) },
    { title: '异议(笔)', dataIndex: 'objectionCount', key: 'objectionCount', render: (objectionCount, record) => renderTransactions(objectionCount, record, 3) },
