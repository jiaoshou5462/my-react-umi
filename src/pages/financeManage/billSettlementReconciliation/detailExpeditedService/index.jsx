...
const detailExpeditedService = (props) => {
      ...
      <Row className={style.detail_box}>
        ...
        <Col span={12} className={style.col_item}>
          <div className={style.detail_item}>
            ...
            <Row className={style.detail_row}>
              ...
              <Col span={8} className={style.detail_col}>
                <div className={style.title_fsz2}>详细地址</div>
        ...
        <Col span={12} className={style.col_item}>
          <div className={style.right_box}>
            ...
            {/* 办理完成照片 */}
            {
              balanceDetailFact.motOrderType ?
                balanceDetailFact.motOrderType == 17 || balanceDetailFact.motOrderType == 30 ?
                  <div className={style.right_phone_box}>
                    <div className={style.right_phone_title}>办理完成图片</div>
                    <Rotation rotationList={balanceDetailFact.completedImageList} />
                  </div> : null
                : null
            }
            {/* 损伤照片 */}
            {
              balanceDetailFact.motOrderType ?
                balanceDetailFact.motOrderType == 17 || balanceDetailFact.motOrderType == 30 ?
                  <div className={style.right_phone_box}>
                    <div className={style.right_phone_title}>损伤照片</div>
                    <Rotation rotationList={balanceDetailFact.damageImageList} />
                  </div> : null
                : null
            }
            {/* 证件照片 */}
            {
              balanceDetailFact.certificateImageList && balanceDetailFact.certificateImageList.length > 0 || (balanceDetailFact.motOrderType == 15 && (balanceDetailFact.orderStatus == 0 || balanceDetailFact.orderStatus == 9)) ?
                <div className={style.right_phone_box}>
                  <div className={style.right_phone_title}>证件照片</div>
                  <Rotation rotationList={balanceDetailFact.certificateImageList} />
                </div> : null
            }
