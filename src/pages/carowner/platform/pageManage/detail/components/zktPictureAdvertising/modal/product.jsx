...
import {
  Input, Image, Space,Form,Table,Button,Modal,Select
} from "antd";
  ...
  const rowSelection = {
    onChange: (key, option) => {
      rowOption = option.map(item=>{
        //替换 弹窗抛出给外部的数据
        return {value:item.crmObjectId,label:item.goodTitle};
      });
    ...
    <Modal title={'热销产品'} width={1200} visible={true} centered maskClosable={false} onOk={selectSend} onCancel={handleCancel}>
      <div className={style.information}>
        ...
        onChange={handleTableChange}>
          ...
          rowSelection={rowSelection} rowKey="crmObjectId">
            <Column title="产品ID" dataIndex="objectId" key="objectId" width={100}/>
            <Column title="产品名称" dataIndex="goodTitle" key="goodTitle" />
            <Column title="产品图片" dataIndex="goodImg"  key="goodImg"
            render={(text, record) => (
