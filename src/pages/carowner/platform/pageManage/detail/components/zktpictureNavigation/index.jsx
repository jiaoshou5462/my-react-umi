...
import Information from "../zktPictureAdvertising/modal/information";
import Activity from "../zktPictureAdvertising/modal/activity";
import Product from "../zktPictureAdvertising/modal/product";
...
const zktPictureAdvertising = (props) => {
  ...
  let setMcolor = (n, i) => {//名称，颜值
    let toFormDataStyle = JSON.parse(JSON.stringify(formDataStyle));
    toFormDataStyle[n] = i;
    console.log(toFormDataStyle)
    setFormDataStyle(toFormDataStyle);
  };
  //重置颜色
  const resetColor=(name)=>{
    let toFormDataStyle = JSON.parse(JSON.stringify(formDataStyle));
    let resetStr = name=='backgroundColor' ? '#fff' : '#333';
  ...
  return (
    <div className={style.wrap}>
      <Tabs defaultActiveKey="1" activeKey={activeKey} onTabClick={setActiveKey} className={style.wrap_tab}>
        <TabPane tab="图文导航内容" key="1">
          <div className={style.wrap_box}>
            <div className={style.wrap_child2}>
              <h5>上传图片</h5>
              <p>要求：图片宽度60px 高度60px，图片大小不超过200kb，支持图片格式jpg/png。</p>
                    ...
                    return <div className={style.wrap_list_li}>
                      ...
                      <div className={style.wrap_list_li_info}>
                        <div className={style.wrap_list_li_item}>
                          <span>标题：</span><Input onChange={(e)=>{inputChange(index,e.target.value)}} maxLength="4" 
                          className={style.item_input} value={item.title} placeholder="请输入标题 (最多四个字)"/>
                        </div>
                        <div className={style.wrap_list_li_item}>
          ...
          <div className={style.wrap_box2}>
            ...
            <div className={style.wrap_box2_main}>
              <div className={style.wrap_box2_p}>
                <strong>导航样式</strong>
                <div className={style.wrap_box2_top}>显示<InputNumber min={1} max={5} style={{width:'100px',margin:'0 10px',marginTop:'-10px',}}
                    value={formDataStyle.lineNum} onChange={numChange} disabled={formDataStyle.bannerStyle=='1'}></InputNumber>个（超出的导航以横向滑动的方式展示）
                </div>
                {/* <Radio.Group className={style.wrap_box2_top2} onChange={styleChange} value={formDataStyle.bannerStyle}>
                  <Radio value={'1'} className={style.wrap_box2_pro}>横向滑动</Radio>
                  <Radio value={'2'} className={style.wrap_box2_pro}>显示
                    </Radio>
                </Radio.Group> */}
              </div>
              <div className={style.wrap_box2_p}>
                <strong>背景颜色</strong>
                <div className={style.wrap_box2_top}><SetColor colors={formDataStyle.backgroundColor} colorName='backgroundColor' setMColor={setMcolor}></SetColor></div>
                <span className={style.reset_btn} onClick={()=>{resetColor('backgroundColor')}}>重置</span>
              </div>
              <div className={style.wrap_box2_p}>
                <strong>文字颜色</strong>
                <div className={style.wrap_box2_top}> <SetColor colors={formDataStyle.textColor} colorName='textColor' setMColor={setMcolor}></SetColor></div>
                <span className={style.reset_btn} onClick={()=>{resetColor('textColor')}}>重置</span>
              </div>
            </div>
          </div>
        </TabPane>
