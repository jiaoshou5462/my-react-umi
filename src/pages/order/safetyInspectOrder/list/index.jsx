...
const safetyInspectOrderListPage =(props)=>{
  ...
  let getList = () => {
    dispatch({
      type: 'safetyInspectOrderList/getList',
      payload: {
        method: 'postJSON',
        params: payload
      },
  ...
  let operateRender = (objectId) => {
    ...
    let goToDetail = () => {
      history.push({
        pathname: '/order/safetyInspectOrder/list/detail',
        state: {
          objectId
        }
      })
  ...
  /*实际门店页数方法 点击下一页上一页*/
  let onActualStoreNextChange = (page, pageSize) =>{
    ...
    <div>
      <div className={style.filter_box}>
        <QueryFilter className={style.form} form={form} defaultCollapsed onFinish={searchBtnEvent} onReset={resetBtnEvent}>
          ...
          <Form.Item label="预约检测时间：" name="bookTime" labelCol={{flex: '0 0 120px'}}>
            <RangePicker
                allowClear
                locale={locale}
                style={{width: '100%'}}
          ...
          <Form.Item label="预约检测门店：" name="bookLocationId" labelCol={{flex: '0 0 120px'}}>
            <Select
                allowClear
                placeholder="不限"
...
export default connect(({safetyInspectOrderList, orderPublic})=>({
  list: safetyInspectOrderList.list,
  providerList: orderPublic.providerList,
  pageTotal: safetyInspectOrderList.pageTotal,
