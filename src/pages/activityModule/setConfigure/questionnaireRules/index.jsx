...
import { Button, Row, Space, Select, Checkbox, Tooltip, Form, Radio, InputNumber, Col, Input, Upload, message, DatePicker, Modal, Switch, Table, Rate } from 'antd';
import { connect, history } from 'umi';
const { TextArea } = Input;
import styles from './style.less';
import { InfoCircleOutlined, UploadOutlined, MinusCircleOutlined } from '@ant-design/icons'
...
const configActivityRulesPage = (props) => {
  ...
  let [itemsList, setItemsList] = useState();//添加题目选项
  ...
  let topicColumns = [
    {
      ...
      dataIndex: 'choseType',
      width: '10%',
      render: (choseType) => choseTypeRander(choseType)
    }, {
      title: '题目名称',
      dataIndex: 'name',
      width: '30%',
  ...
  let editSubjectFun = (item) => {
    setEditInfo({ ...item });
    let toTopicInfo = {};
    toTopicInfo.topicType = item.choseType;
    toTopicInfo.name = item.name;
    let toItemsList = JSON.parse(JSON.stringify(item.itemList));
    formTopic.setFieldsValue({
      choseType: item.choseType
    });
    setTopicInfo({ ...toTopicInfo });
    setItemsList(toItemsList);
  ...
  let topicFinish = (value) => {
    let toItemList = JSON.parse(JSON.stringify(itemsList));
    if(!topicInfo.name){
      message.error('请输入题目！');
      return
    }
    let showMsg = false;
    toItemList.length > 0 && toItemList.map(item => {
      if(!item.itemName){
        showMsg = true;
      }
    })
    if((value.choseType == 0 || value.choseType == 1) && showMsg){
      message.error('请输入选项！');
      return
    }
    let toParams = {}
  ...
  let topicSelec = (e, name) => {
    let toItemsList = JSON.parse(JSON.stringify(itemsList));
    if((e == 0 || e == 1) && toItemsList.length < 2){
      toItemsList.push({
        itemAnswer: 0,
        itemName: ""
      });
  ...
  //题目输入
  let selectionNameChange = (e) => {
    let value = e.target.value;
    let toTopicInfo =JSON.parse(JSON.stringify(topicInfo));
    toTopicInfo.name = value;
    setTopicInfo(toTopicInfo);
    formTopic.setFieldsValue({
      name: value
    })
  ...
  //单选多选删除选项
  let delSelectionClick = (e, key) => {
    let toItemList = JSON.parse(JSON.stringify(itemsList));
    toItemList = toItemList.filter((item,index) => {return index != key})
    setItemsList(toItemList);
  }
  let selectionValueChange = (e, index) => {
    let value = e.target.value;
    let toItemList = JSON.parse(JSON.stringify(itemsList));
    toItemList[index].itemName = value;
    setItemsList(toItemList);
  }
  //添加题目
  let addTopic = () => {
  ...
  let onFinish = (value) => {
    if (!detailStatus) {
      ...
      toForm.drawType = toForm.drawType ? toForm.drawType : 1;
      // 触发动作
      ...
      let params = {
        ...
        gameAnswerConfigVO: {
          ...
          subSuccessWord: toForm.subSuccessWord,
          gameNumber: 1
        }
      }
      dispatch({
        ...
        callback: (res) => {
          if (res.result.code === '0') {
            if (isDataChange && isModalVisible) {
              ...
              toAtInfo.marketActivityType = 11;
      ...
      callback: (res) => {
        if (res.result.code === '0') {
          ...
          form.setFieldsValue({
            ...
            subSuccessWord: items.subSuccessWord
          });
        ...
        <Form form={formTopic} onFinish={topicFinish} labelCol={{ span: 6 }} wrapperCol={{ span: 18 }}>
          <Form.Item name="choseType" label="题目类型">
            <Select onChange={(e) => { topicSelec(e, 'topicType') }}>
              ...
              <Option value={3}>打分题</Option>
            </Select>
          </Form.Item>
          <div className={styles.selection_name}>
            <div className={styles.selection_label}><span>* </span>题目：</div>
            <Input className={styles.selection_input} value={topicInfo.name} onChange={(e) => {selectionNameChange(e)}} style={{ width: 354 }}></Input>
          </div>
          {
            topicInfo.topicType == 0 || topicInfo.topicType == 1 ?    //单选
              <div>
                {
                  itemsList && itemsList.length > 0 && itemsList.map((item, index)=>{
                    return <div className={styles.selection_item}>
                      {index > 1 ? <div className={styles.selection_del} onClick={(e) =>{delSelectionClick(e, index)}}><MinusCircleOutlined /></div> : null}
                      <div className={styles.selection_label}><span>* </span>选项{index + 1}：</div>
                      <Input className={styles.selection_input} value={item.itemName} onChange={(e) => {selectionValueChange(e, index)}} style={{ width: 354 }}></Input>
                    </div>
                  })
                }
                <div className={styles.selection_add} onClick={addSelectionClick}>+添加单个选项</div>
              </div> 
              : topicInfo.topicType == 2 ?  //填空
              <div className={styles.wrap_layer4}>
                <TextArea autoSize={{ minRows: 3, maxRows: 5 }} disabled/>
              </div>
              : //打分
              <div className={styles.wrap_layer5}>
                <Rate style={{color: '#0091FF'}} disabled defaultValue={2} />
              </div>
          }
        ...
        <div className={styles.mould_wrap}>
          ...
          <div className={`${styles.wrap2_box} ${styles.wrap2_box2}`}>
            <div className={styles.wrap2_box2_by}>
              <span className={styles.wrap2_box2_by_n1}>问卷完成后是否开启抽奖：</span>
              ...
              isLuckDraw ?
                <div className={styles.wrap2_box2_by2}>
                  ...
                  <div className={styles.wrap2_box2_content}>
                    <Form.Item extra={<span className={styles.box2_count1_tip}>次</span>} name="drawPoints" label="抽奖次数" className={styles.box2_count1} rules={[{ required: true, message: "请输入" }]}>
                      <InputNumber disabled={true} className={styles.box2_count_width1} min={0} />
                    </Form.Item>
                  </div>
          ...
          <div className={`${styles.wrap2_box} ${styles.wrap2_box7}`}>
            ...
            {/* <Button type="primary" disabled={detailStatus || isActivityHave} onClick={addImport}>批量导入</Button> */}
            <h3><span>题目列表</span><span className={styles.wrap2_box7_h1}><Button type="primary" disabled={detailStatus || isActivityHave}  onClick={addTopic}>添加题目</Button></span></h3>
            ...
            <Form.Item name="subSuccessWord" label="提交成功文案设置" className={styles.wrap2_box7_n1} rules={[{ required: true, message: "请输入提交成功文案" }]}>
              <Input disabled={detailStatus || isActivityHave} placeholder="请输入" />
            </Form.Item>
