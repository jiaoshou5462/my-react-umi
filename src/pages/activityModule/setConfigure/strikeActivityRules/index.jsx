...
const configActivityRulesPage = (props) => {
  ...
  let [gameNumberType, setGameNumberType] = useState(1);//参与游戏次数类型
  ...
  let getStageSActivityTwo = () => {
    dispatch({
      type: 'strikeActivityRules/getActivityRuleDetail',
      payload: {
        method: 'postJSON',
        params: {
          channelId: activityInfo.channelId,
  ...
  let onFinish = (value) => {
    ...
    if (!detailStatus) {
      ...
      let params = {
        ...
        gameStrikeConfigVO: {
          activityId: activityInfo.objectId,
          countDown: toForm.strikeCountdown,
          drawForm: toForm.strikeLotteryForm,
          gameNumberType: toForm.gameNumberType,
          gameNumber: toForm.gameNumber,
          maxScore: "",
          openDraw: toForm.isStrikeLottery,
          // showNumber: isShowParticipants == 1 ? 1 : 0,
          targetRatio: "",
          targetScore: "",
        }
      }
      dispatch({
        type: 'strikeActivityRules/activityRule',
        payload: {
          method: 'postJSON',
          params: params
        },
  ...
  let getStrikeConfig = () => {
    dispatch({
      ...
      callback: (res) => {
        if (res.result.code === '0') {
          let items = res.body;
          if(items.gameNumberType==2){//如果不不限次数，则清空
            items.gameNumber = '';
          }
          form.setFieldsValue({
            strikeCountdown:items.countDown,//游戏时间
            isStrikeLottery:items.openDraw ? items.openDraw : 0,//是否开启抽奖
            strikeLotteryForm:items.drawForm ? items.drawForm : 3,//抽奖形式
            gameNumberType:items.gameNumberType ? items.gameNumberType : 0,//参与游戏次数类型
            gameNumber:items.gameNumber ? items.gameNumber : 1,//参与游戏次数
          });
          setStrikeLottery(items.openDraw ? items.openDraw : 0)
  ...
  let [isStrikeLottery,setStrikeLottery]=useState(1);//是否开启抽奖 默认开启
  let strikeLotteryChange = (i) => {
    let strikeLottery  = i.target.value;
    setStrikeLottery(strikeLottery);
    form.setFieldsValue({
      isStrikeLottery:strikeLottery
    })
  }
  let changeLotteryForm = (i) => {
    let lotteryForm = i.target.value;
    form.setFieldsValue({
      strikeLotteryForm:lotteryForm
    })
  }
  useEffect(() => {
    form.setFieldsValue({
      ...
      strikeCountdown:null,//游戏时间
      isStrikeLottery:1,//是否开启抽奖
      strikeLotteryForm:3,//抽奖形式
      gameNumberType:1,//参与游戏次数类型
      gameNumber:1,//参与游戏次数
      nDuringType:1,
    })
  }, []);
    ...
    <>
      ...
      <Form form={form} onFinish={onFinish} onFieldsChange={onFieldsChange}>
        ...
        <div className={styles.mould_wrap}>
          ...
          <div className={`${styles.wrap2_box} ${styles.wrap2_box2}`}>
          <h3>游戏规则设定</h3>
            <Form.Item name="strikeCountdown" label="游戏时间：" className={styles.box2_count1} rules={[{ required: true , message:"请输入游戏时间" }]}>
              <InputNumber disabled={detailStatus || isActivityHave} parser={limitNumber} formatter={limitNumber} className={styles.box2_count_width1} min={0} />
            </Form.Item>
            <p className={styles.notice}>注：游戏时间会影响物品下坠间隔的频率，时间越短物品出现的越多，时间越长物品出现的越少。</p>
            <Form.Item name="gameNumberType" label="参与游戏次数：" className={styles.box2_radio} rules={[{ required: true }]}>
              <Radio.Group disabled={detailStatus || isActivityHave} onChange={changeGameNumberType}>
                ...
                <Radio value={2}>不限次数</Radio>
              </Radio.Group>
            </Form.Item>
            {gameNumberType != 2 ? 
            <Form.Item name="gameNumber" label="" className={styles.box2_count} extra={gameNumberType == 0 ? '次日的游戏次数会重置为该值' : '活动期间每个用户总共的游戏次数'} rules={[{ required: true, message: "请输入次数" }]}>
              <InputNumber disabled={detailStatus || isActivityHave} className={styles.box2_count_width} min={1} />
            </Form.Item>
            : <p className={styles.notice}>活动期间每个用户可以无限制的参与游戏</p>}
            <Form.Item name="isStrikeLottery" label="完成目标分后是否开启抽奖" className={styles.box2_radio} rules={[{ required: true }]}>
              <Radio.Group disabled={detailStatus || isActivityHave} value={isStrikeLottery} onChange={strikeLotteryChange}>
                <Radio value={1}>是</Radio>
                <Radio value={0}>否</Radio>
              </Radio.Group>
            </Form.Item>
            {isStrikeLottery == 1 ? 
            <Form.Item name="drawType" label="抽奖次数限制：" className={styles.box2_radio} rules={[{ required: true }]}>
            ...
            : null}
            {isStrikeLottery == 1 ? 
            <Form.Item name="drawPoints" label="" className={styles.box2_count} extra={drawTypes == 0 ? '次日的抽奖次数会重置为该值' : '活动期间每个用户总共的抽奖次数'} rules={[{ required: true, message: "请输入次数" }]}>
              <InputNumber disabled={detailStatus || isActivityHave} className={styles.box2_count_width} min={1} />
            </Form.Item>
            : null}
            {isStrikeLottery == 1 ? 
              <Form.Item name="strikeLotteryForm" label="抽奖形式：" className={styles.box2_radio} rules={[{ required: true }]}>
                <Radio.Group disabled={detailStatus || isActivityHave} onChange={changeLotteryForm}>
                  <Radio value={3}>直抽</Radio>
                  <Radio value={7}>其他活动</Radio>
                </Radio.Group>
              </Form.Item>
            : null}
          </div>
          ...
          <div className={`${styles.wrap2_box} ${styles.wrap2_box3}`}>
            ...
            <div className={styles.box3_main}>
              <Form.Item name="luckTask" label="增加游戏任务：">
          ...
          <div className={`${styles.wrap2_box} ${styles.wrap2_box5}`}>
            ...
            <div className={styles.box5_main}>
              ...
              {/* <div className={styles.box5_main_li}>
                ...
                <Switch checked={isShowParticipants === 1} onChange={setIsShowParticipants} />
              </div> */}
              <div className={styles.box5_main_li}>
...
export default connect(({ strikeActivityRules, loading, activityModule, twoNumber }) => ({
