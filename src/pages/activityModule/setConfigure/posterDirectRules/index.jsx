...
const configActivityRulesPage = (props) => {
  ...
  let getStageSActivityTwo = () => {
    dispatch({
      type: 'posterDirectRules/getActivityRuleDetail',
      payload: {
        method: 'postJSON',
        params: {
          channelId: activityInfo.channelId,
  ...
  let onFinish = (value) => {
    if (!detailStatus) {
      ...
      dispatch({
        type: 'posterDirectRules/activityRule',
        payload: {
          method: 'postJSON',
          params: params
        },
        callback: (res) => {
          if (res.result.code === '0') {
            if (isDataChange && isModalVisible) {
              ...
              toAtInfo.marketActivityType = 10;
  ...
  useEffect(()=>{
    //数据回显
    if (activityInfo && activityInfo.objectId && activityInfo.channelId && ruleList && ruleList.length > 0) {
...
export default connect(({ posterDirectRules, loading, activityModule, twoNumber }) => ({
