...
import { Button, Row, Space, Select, Checkbox, Tooltip, Form, Radio, InputNumber, Col, Input, Upload, message, DatePicker, Modal, Switch, TimePicker } from 'antd';
...
const signInRulesPage = (props) => {
  ...
  let [useTimeDrawOpen, setUseTimeDrawOpen] = useState(0); //参与抽奖次数
  let changeUseTimeDrawOpen = (i) => {
    let value = i.target.value;
    setUseTimeDrawOpen(value);
    form.setFieldsValue({
      useTimeDrawOpen: value
    })
  }
  let [playWay, setPlayWay] = useState([]); //操作签到玩法
  let [showPlayWay, setShowPlayWay] = useState([]); //展示签到玩法
  let changePlayWay = (val) => {
    setPlayWay(val);
  }
  let [rewardRuleList, setRewardRuleList] = useState([]);//操作签到规则
  let [showRuleList, setShowRuleList] = useState([]);//展示签到规则
  let [isEditRules, setIsEditRules] = useState(false);//编辑规则弹窗
  //保存规则
  let saveRules = () => {
    let list = JSON.parse(JSON.stringify(rewardRuleList));
    if(playWay.length < 1){
      message.info('请选择玩法');
      return
    }
    let filterDelList = list.length > 0 && list.filter(item => {return item.deleted == 2})
    if(filterDelList.length < 1){
      message.info('请添加规则');
      return
    }
    let isEdit = false;
    ...
    list.map(item => {
      if(item.deleted == 2){
        originList.push(item.days)
      }
    });
    let fialList =[...(new Set(originList))];
    if(originList.length != fialList.length){
      message.info('有重复签到规则，请重新配置');
      return
    }
    saveOrUpdateActivityRule(list);
  }
  //关闭编辑规则弹窗
  let closeEditRules = () => {
  ...
  //编辑规则
  let editRulesClick = () => {
    setRewardRuleList(showRuleList);
    if(detailStatus || isActivityHave){
      return
    }
    setIsEditRules(true);
  }
  ...
  let addRulesClick = () => {
    ...
    setRewardRuleList(list);
  }
  //输入规则天数
  let ruleDaysChange = (e, index) => {
    let list = JSON.parse(JSON.stringify(rewardRuleList));
    list[index].days = e;
    setRewardRuleList(list);
  }
  //删除规则
  let delRewardRule = (e, key) => {
    let list = JSON.parse(JSON.stringify(rewardRuleList));
    if(!list[key].id){
      list = list.filter((item,index) => {return index != key});
      setRewardRuleList(list);
    }else{
      dispatch({
        type: 'signInRules/checkActivityRule',
        payload: {
          method: 'get',
          params: {
            activityId: activityInfo.objectId,
        ...
        callback:(res)=>{
          if (res.result.code == "0") {
            if(res.body){
              Modal.confirm({
                content: `此条规则已被${res.body}关联，删除后会解除关联关系，确认是否删除?`,
                cancelText: '否',
                okText: '是',
                onOk: () => {
                  list[key].deleted = 1;
  ...
  //查询规则
  let checkActivityRule = (list, key) => {
    dispatch({
      type: 'signInRules/checkActivityRule',
      payload: {
        method: 'get',
      ...
      callback:(res)=>{
        if (res.result.code == "0") {
          if(res.body == 0){
            Modal.confirm({
              content: '当前规则已关联奖池人群，若删除规则将同步取消关联，是否确认删除？',
              okText: '确定',
              onOk () {
                list[key].deleted = 1;
                setRewardRuleList(list);
  ...
  let getActivityRule = () => {
    dispatch({
      type: 'signInRules/getActivityRule',
      payload: {
        method: 'get',
        params: {
          activityId: activityInfo.objectId
      ...
      callback:(res)=>{
        if (res.result.code == "0") {
          if(res.body.activityId){
            setPlayWay(res.body.types);
            setRewardRuleList(res.body.details);
            setShowPlayWay(res.body.types);
            setShowRuleList(res.body.details);
          }
        }else{
          message.error(res.result.message);
        }
  ...
  //查询规则
  let saveOrUpdateActivityRule = () => {
    dispatch({
      type: 'signInRules/saveOrUpdateActivityRule',
      payload: {
        method: 'postJSON',
        params: {
          activityId: activityInfo.objectId,
  ...
  let [totalDayNum, setTotalDayNum] = useState(0);//签到周期总天数
  //数据获取
  let getStageSActivityTwo = () => {
    dispatch({
      type: 'goldenEggsRules/getActivityRuleDetail',
      payload: {
        method: 'postJSON',
        params: {
          channelId: activityInfo.channelId,
      ...
      callback: (res) => {
        if (res.result.code == "0") {
          ...
          let toStartTime = formatTime(toItemObj.startTime).split(' ')[0];
          let toEndTime = formatTime(toItemObj.endTime).split(' ')[0];
          let totalDayNum = moment(toEndTime).diff(moment(toStartTime),'days') + 1;
          setTotalDayNum(totalDayNum);
          toItemObj.activityTime = [moment(toItemObj.startTime),moment(toItemObj.endTime)];
          if(toItemObj.drawOpenStartTime && toItemObj.drawOpenEndTime){
            toItemObj.openTime = [moment(toItemObj.drawOpenStartTime, 'HH:mm:ss'),moment(toItemObj.drawOpenEndTime, 'HH:mm:ss')];
          }
          setUseTimeDrawOpen(toItemObj.useTimeDrawOpen);
  ...
  let onFinish = (value) => {
    if (!detailStatus) {
      ...
      if(showRuleList.length < 1){
        message.info('请设置玩法及规则')
        return
      }
      ...
      let params = {
        ...
        activityRuleBaseInfoVO: {
          showDraw: toForm.jackpot,
          useTimeDrawOpen: toForm.useTimeDrawOpen,
          drawOpenStartTime:toForm.openTime ? moment(toForm.openTime[0]).format('HH:mm:ss') : '',
          drawOpenEndTime:toForm.openTime? moment(toForm.openTime[1]).format('HH:mm:ss') : '', 
      ...
      dispatch({
        type: 'goldenEggsRules/activityRule',
        payload: {
          method: 'postJSON',
          params: params
        },
        callback: (res) => {
          if (res.result.code === '0') {
            if (isDataChange && isModalVisible) {
              ...
              toAtInfo.marketActivityType = 12;
              toAtInfo.marketType = 1;
  ...
  let [jackpot, getJackpot] = useState(0);//是否开启奖池展示
  let setJackpot = (value) => {
    let jackpot = value ? 1 : 0;
    getJackpot(jackpot);
  }
  //企业logo
  ...
  return (
    <>
      {/* 编辑规则 */}
      <Modal title="设置玩法及规则" visible={isEditRules} footer={null} onCancel={closeEditRules}>
        <div className={styles.editRule_wrap}>
          <div className={styles.rules_play}>
            <div className={styles.play_title}><span className={styles.star}>*</span>选择玩法：</div>
            <Checkbox.Group className={styles.rules_check} value={playWay} onChange={changePlayWay}>
              <Checkbox value={1}>连续签到</Checkbox>
              {/* <Checkbox value="2">累计签到</Checkbox> */}
            </Checkbox.Group>
          </div>
          <div className={styles.rules_content}>
            {rewardRuleList && rewardRuleList.length > 0 && rewardRuleList.map((item,index) => {
              return <>{item.deleted == 2 ? <div className={styles.rules_item}>
              <div>规则{index+1}：连续签到 <InputNumber onChange={(e) => ruleDaysChange(e, index)} disabled={detailStatus || isActivityHave} value={item.days} className={styles.box2_count_width} min={1} max={totalDayNum}/> 天</div>
              <div className={styles.rule_del} onClick={(e) => {delRewardRule(e,index)}}>删除</div>
            </div> : null}</>
            })}
          </div>
          <div className={styles.notice}>注：修改规则，奖池关联规则将同步更新！</div>
          <div className={styles.btn} onClick={addRulesClick}>+添加</div>
        </div>
        
        <div className={styles.modal_btn}>
          <Button onClick={closeEditRules}>取消</Button>
          <Button type="primary" onClick={saveRules}>保存</Button>
        </div>
      </Modal>
      {/* 上一步弹窗 */}
      ...
      <Form form={form} onFinish={onFinish} onFieldsChange={onFieldsChange}>
        ...
        <div className={styles.mould_wrap}>
          ...
          <h3>签到规则</h3>
            <div className={styles.activityTime_wrap}>
              <Form.Item name="activityTime" label="活动时间" className={`${styles.form__item} ${styles.activityTime}`}>
                <RangePicker locale={locale} disabled={true} showTime format="YYYY-MM-DD" className={styles.form_item_width1}/>
              </Form.Item>
              <div className={styles.totalDay}>共计{totalDayNum}天</div>
            </div>
            <div className={styles.rewardRules_wrap}>
              <div className={styles.rules_top}>
                <div className={styles.title}><span className={styles.star}>*</span>设置玩法及规则<Tooltip title="满足规则即可获得1次抽奖机会"><InfoCircleOutlined className={styles.wrap2_ico} /></Tooltip>：</div>
                <div className={styles.addRules} onClick={editRulesClick}>设置</div>
              </div>
              {showPlayWay && showPlayWay.length > 0 ? <div className={styles.rule_play}>
                <div className={styles.play_label}>签到玩法：</div>
                <Checkbox.Group className={styles.rules_check} value={showPlayWay} disabled={true}>
                  <Checkbox value={1}>连续签到</Checkbox>
                  {/* <Checkbox value="2">累计签到</Checkbox> */}
                </Checkbox.Group>
              </div> : null}
              <div className={styles.rules_content}>
                {showRuleList && showRuleList.length > 0 && showRuleList.map((item,index) => {
                  return <>{item.deleted == 2 ? <div className={styles.rules_item}>
                  <div>规则{index+1}：连续签到{item.days}天</div>
                </div> : null}
                  </>
                })}
              </div>
          ...
          <div className={`${styles.wrap2_box} ${styles.wrap2_box2}`}>
            <h3>奖池及中奖率</h3>
            <div>
              <Form.Item name="useTimeDrawOpen" label="是否指定奖池开放时间：" className={styles.box2_radio} rules={[{ required: true }]}>
                <Radio.Group disabled={detailStatus || isActivityHave} onChange={changeUseTimeDrawOpen}>
                  <Radio value={1}>是</Radio>
                  <Radio value={0}>否</Radio>
                </Radio.Group>
              </Form.Item>
              {useTimeDrawOpen == 1 ? <Form.Item name="openTime" label="选择时间" className={styles.form__item} rules={[{required: true, message: "请选择奖池开放时间" }]}>
                <RangePicker picker='time' allowClear={false} locale={locale} disabled={detailStatus||isActivityHave} showTime format="HH:mm:ss" className={styles.form_item_width1}/>
              </Form.Item>
              : null}
          ...
          <div className={`${styles.wrap2_box} ${styles.wrap2_box5}`}>
            ...
            <div className={styles.box5_main}>
              ...
              <div className={styles.box5_main_li}>
                <em className={styles.box5_main_li_em}>奖池展示：<Tooltip title="开关控制C端的奖池模块的是否展示"><InfoCircleOutlined className={styles.wrap2_ico} /></Tooltip></em>
                <Switch checked={jackpot === 1} onChange={setJackpot} />
              </div>
            </div>
          </div>
          {/* 订阅消息通知 */}
...
export default connect(({ goldenEggsRules, loading, activityModule, twoNumber }) => ({
