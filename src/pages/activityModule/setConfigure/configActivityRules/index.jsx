...
const configActivityRulesPage = (props) => {
  ...
  let [postFrom, setPostFrom] = useState({ //接口相关参数
    ...
    [drawTypes, setDrawTypes] = useState(0); //参与抽奖次数
  ...
  let getStageSActivityTwo = () => {
    dispatch({
      type: 'configActivityRules/getActivityRuleDetail',
      payload: {
        method: 'postJSON',
        params: {
          channelId: activityInfo.channelId,
      ...
      callback: (res) => {
        if (res.result.code == "0") {
          ...
          if (activityInfo.marketActivityType) {//活动未配置规则时回显触发动作不参与
            ...
            toIsTools[7] = toItemObj.inviteFriends == 1;
            setisTools([...toIsTools]);
          }
  ...
  let onFinish = (value) => {
    if (!detailStatus) {
      ...
      dispatch({
        type: 'configActivityRules/activityRule',
        payload: {
          method: 'postJSON',
          params: params
        },
  ...
  useEffect(() => {
    ...
    if (ruleList && ruleList.length > 0) {
      ...
      let toPartakAt = [];
      let toFieldsValue = form.getFieldsValue();
      ...
      toFieldsValue.partakAt = toPartakAt;
      form.setFieldsValue({ ...toFieldsValue });
      setTriggerActionList({ ...toTriggerActionList });
    }
  }, [ruleList, twoInfo]);
      ...
      <Form form={form} onFinish={onFinish} onFieldsChange={onFieldsChange}>
        ...
        <div className={styles.mould_wrap}>
          ...
          <div className={`${styles.wrap2_box} ${styles.wrap2_box2}`}>
            <Form.Item name="drawType" label="参与抽奖次数：" className={styles.box2_radio} rules={[{ required: true }]}>
            ...
            <Form.Item name="drawPoints" label="" className={styles.box2_count} extra={drawTypesExtra[drawTypes]} rules={[{ required: true, message: "请输入次数" }]}>
              <InputNumber disabled={detailStatus || isActivityHave} className={styles.box2_count_width} min={0} />
            </Form.Item>
          </div>
          {/*将池及中奖率*/}
          <div className={`${styles.wrap2_box} ${styles.wrap2_box2} ${styles.wrap2_box11}`}>
            <h3>奖池及中奖率</h3>
            <h3>
          ...
          {/* n中1 */}
          <div className={`${styles.wrap2_box} ${styles.wrap2_box12}`}>
...
export default connect(({ configActivityRules, loading, activityModule, twoNumber }) => ({
