...
const gamePagePreview = (props) => {
  ...
  let previewGamePage = ()=>{
      ...
      setInviteFriendsNum(items.inviteFriendsNum?tems.inviteFriendsNum:0)
      setSpecifyLinkNum(items.specifyLinkNum);
      let togameColor = gameColor;
      togameColor.color1 = items.belowBackgroundColor || '#fff'
      togameColor.color2 = items.turntableLuckyDrawFontColor || '#fff'
      togameColor.color3 = items.turntableAdTitleFontColor || '#fff'
          ...
          <div className={style.wrap_preview}>
            <div className={style.phone_wrap} style={{ 'height': isShowFirend && isShowShare && isShowAdver ? '1014px' : (isShowShare && isShowAdver) || (isShowFirend && isShowShare) ? '914px' : isShowShare || isShowAdver ? '777px' : '671px' }}>
              ...
              <div className={style.index_wrap} style={{ "backgroundColor": gameColor.color1, 'height': isShowFirend && isShowShare && isShowAdver ? "907px" : (isShowShare && isShowAdver) || (isShowFirend && isShowShare) ? '807px' : isShowShare || isShowAdver ? '670px' : '566px' }}>
                ...
                <div className={style.phone_bombox} >
                  {isShowFirend ? <div className={`${style.phone_share} ${style.phone_hover} ${style.phone_share_firend}`} style={{overflow:'hidden'}} onClick={setTools.bind(this, 6)}>
                    {
                  ...
                  {isShowShare ? <div className={`${style.phone_share} ${style.phone_hover}  ${style.phone_share_bg} `} style={{background:shareType == 1?'rgba(0,0,0,0.37)': shareType == 2?'#0023EE': shareType == 3?'#E53423':shareType == 4?'#790000':'url('+shareType+') no-repeat 0 0 /100% auto' }}  onClick={setTools.bind(this, 4)}>
                    ...
                    : null}
                  {
                    isShowAdver ? <div className={`${style.phone_adver} ${style.phone_hover}`} style={{ background: isTaskStyle == 1 ? 'rgba(0, 0, 0, 0.37)' : isTaskStyle == 2 ? '#0023EE' : isTaskStyle == 3 ? '#E53423' : isTaskStyle == 4 ? '#790000' : 'url('+isTaskStyle+') no-repeat 0 0 /100% auto' }} onClick={setTools.bind(this, 5)}>
                      {adverIsTit == 1 ?
