...
import DiscountHome from '../../components/discountHome';   //首页
import DiscountGame from '../../components/discountGame';   //转盘页
import DiscountList from '../../components/discountList';   //转盘页
...
const activityPage = (props) => {
  ...
  return (
    <div className={styles.content_by}>
      {themeShow ? <SelectTheme themeShow={themeShow} styleCodeType='discountHome' materialCode="discountPage_material" onHideSelectTheme={onHideSelectTheme}/> : null}
      {/* 左侧导航 */}
      <div className={styles.side_left}>
        <div className={styles.side_left_wrap}>
          ...
          <div className={`${styles.side_lf_box1} ${styles.side_lf_box}`}>
            <Button size='large' type={pageInt === 1 ? 'primary ' : 'default'} onClick={setInt.bind(this, 1)}>首页</Button>
            <div><img src={require('../../../../assets/activity/setpage_m4.png')} /></div>
            <Button size='large' type={pageInt === 7 ? 'primary ' : 'ghost'} onClick={setInt.bind(this, 7)}>商品列表页</Button>
            <div><img src={require('../../../../assets/activity/setpage_m4.png')} /></div>
            <Button size='large' type={pageInt === 2 ? 'primary ' : 'ghost'} onClick={setInt.bind(this, 2)}>购买页</Button>
          </div>
        </div>
        <div className={styles.side_left_wrap2}>
          ...
          <div className={styles.side_lf_box}>
            ...
            <Button size='large' type={pageInt === 4 ? 'primary ' : 'default'} onClick={setInt.bind(this, 4)}>购买记录</Button>
          </div>
        </div>
      </div>
      ...
      <div className={styles.side_right}>
        {
          pageInt == 1 ? <DiscountHome onChangType={onChangType} isDataStore={isDataStore} storeConfig={storeConfig} setIsDataStore={setIsDataStore} setIsDataTypes={setIsDataTypes} showLayer={showLayer} isDataChange={isDataChange} />  //首页
            : pageInt == 2 ? <DiscountGame onChangType={onChangType} isDataStore={isDataStore} storeConfig={storeConfig} showLayer={showLayer} isDataChange={isDataChange} /> //购买页
              : pageInt == 3 || pageInt == 4 ? <PublicPops onChangType={onChangType} itemData={itemData} isDataStore={isDataStore} storeConfig={storeConfig} showLayer={showLayer} isDataChange={isDataChange} popTypes={popTypes} styleCodeType="discountHome"/>
                : pageInt == 7 ? <DiscountList onChangType={onChangType} isDataStore={isDataStore} storeConfig={storeConfig} showLayer={showLayer} isDataChange={isDataChange} styleCodeType="discountHome" /> //商品列表页
                  : null
        }
      </div>
    </div>
...
export default connect(({ turntableActivityThree, loading ,selectTheme}) => ({
