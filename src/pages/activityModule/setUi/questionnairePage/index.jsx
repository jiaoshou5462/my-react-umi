...
import VisHome from '../../components/questionnaireHome';   //首页
import VisGame from '../../components/questionnaireGame';   //问卷页
...
const activityPage = (props) => {
  ...
  return (
    <div className={styles.content_by}>
      {themeShow ? <SelectTheme themeShow={themeShow} onHideSelectTheme={onHideSelectTheme}/> : null}
      {/* 左侧导航 */}
      <div className={styles.side_left}>
        <div className={styles.side_left_wrap}>
          <h2 className={styles.wrap_title}>活动页面</h2>
          {/* <span className={styles.select_theme} onClick={()=>{selectThemeClick()}}>选择主题</span> */}
          <div className={`${styles.side_lf_box1} ${styles.side_lf_box}`}>
            ...
            <Button size='large' type={pageInt === 2 ? 'primary ' : 'ghost'} onClick={setInt.bind(this, 2)}>问卷调查</Button>
          </div>
        </div>
      ...
      <div className={styles.side_right}>
        {
          pageInt == 1 ? <VisHome onChangType={onChangType} isDataStore={isDataStore} storeConfig={storeConfig} setIsDataStore={setIsDataStore} setIsDataTypes={setIsDataTypes} showLayer={showLayer} isDataChange={isDataChange} />  //首页
            : pageInt == 2 ? <VisGame onChangType={onChangType} isDataStore={isDataStore} storeConfig={storeConfig} showLayer={showLayer} isDataChange={isDataChange} /> //转盘页
              : pageInt == 3 || pageInt == 4 || pageInt == 5 ? <PublicPops onChangType={onChangType} itemData={itemData} isDataStore={isDataStore} storeConfig={storeConfig} showLayer={showLayer} isDataChange={isDataChange} popTypes={popTypes} styleCodeType='questionnaireHome' />
                : null
        }
      </div>
    </div>
...
export default connect(({ dollMachinePage, loading,selectTheme }) => ({
