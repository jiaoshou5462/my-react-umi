...
const activityPage = (props) => {
  ...
  let gameCancel = (i) => {
    ...
    if (visDataType == 2) {
      ...
      setpageInt(toIntNum);
      setPopTypes(parseInt(toIntNum) - 1);
    }
  }
  let setInt = (i) => {
    if (isDataChange) {
  ...
  let getStageSActivityThree = () => {
    dispatch({
      type: 'activitySeckilPageThree/echoSeckillStyle',
      payload: {
        method: 'postJSON',
        params: {
  ...
  return (
    <div className={styles.content_by}>
      {themeShow ? <SelectTheme themeShow={themeShow} styleCodeType='seckilHome' materialCode="seckilPage_material" onHideSelectTheme={onHideSelectTheme}/> : null}
      ...
      <div className={styles.side_left}>
        ...
        <div className={styles.side_left_wrap2}>
          ...
          <div className={styles.side_lf_box}>
            <Button size='large' type={pageInt === 2 ? 'primary ' : 'default'} onClick={setInt.bind(this, 2)}>活动规则</Button>
            <Button size='large' type={pageInt === 3 ? 'primary ' : 'default'} onClick={setInt.bind(this, 3)}>秒杀记录</Button>
          </div>
        </div>
      </div>
      ...
      <div className={styles.side_right}>
        {
          pageInt == 1 ? <VisHome onChangType={onChangType} itemData={itemData} isDataStore={isDataStore} storeConfig={storeConfig} showLayer={showLayer} isDataChange={isDataChange} />  //首页
            : pageInt == 2 || pageInt == 3 ? <PublicPops onChangType={onChangType} itemData={itemData} isDataStore={isDataStore} storeConfig={storeConfig} showLayer={showLayer} isDataChange={isDataChange} popTypes={popTypes} styleCodeType="seckilHome"/>
              : null
        }
      </div>
    </div>
...
export default connect(({ activitySeckilPageThree, loading,selectTheme }) => ({
  info: activitySeckilPageThree.info,
}))(activityPage);
